#include <stdio.h>
#include "../include/processus.h"

void round_robin(Processus tab[], int n, int quantum) {
    int temps = 0;
    int reste = n;

    printf("\n===== ROUND ROBIN (q=%d) =====\n\n", quantum);
    printf("Gantt :\n");

    // initialisation
    for (int i = 0; i < n; i++) {
        tab[i].restant = tab[i].duree;
        tab[i].debut = -1;
        tab[i].fin = 0;
    }

    while (reste > 0) {
        int actif = 0;

        for (int i = 0; i < n; i++) {

            if (tab[i].restant > 0 && tab[i].arrivee <= temps) {

                actif = 1;

                // enregistrer premier démarrage
                if (tab[i].debut == -1)
                    tab[i].debut = temps;

                int exec = (tab[i].restant < quantum) ? tab[i].restant : quantum;

                for (int t = 0; t < exec; t++) {
                    printf("| %s ", tab[i].nom);
                    temps++;
                }

                tab[i].restant -= exec;

                // si terminé
                if (tab[i].restant == 0) {
                    tab[i].fin = temps;
                    reste--;
                }
            }
        }

        // si aucun processus prêt → avancer le temps
        if (!actif) {
            temps++;
        }
    }

    printf("|\n\n");

    // ===== STATISTIQUES =====
    float total_attente = 0, total_sejour = 0;

    printf("Processus | Arr | Dur | Fin | Att | Sej\n");

    for (int i = 0; i < n; i++) {
        int sejour = tab[i].fin - tab[i].arrivee;
        int attente = sejour - tab[i].duree;

        total_attente += attente;
        total_sejour += sejour;

        printf("%s\t   %d\t %d\t %d\t %d\t %d\n",
               tab[i].nom,
               tab[i].arrivee,
               tab[i].duree,
               tab[i].fin,
               attente,
               sejour);
    }

    printf("\nMoyenne attente = %.2f\n", total_attente / n);
    printf("Moyenne sejour  = %.2f\n", total_sejour / n);
}